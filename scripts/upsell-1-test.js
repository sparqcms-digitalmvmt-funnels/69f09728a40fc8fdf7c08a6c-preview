...
function getNextPageSlugForRedirect() {
  const normalize = (value) => {
    if (!value) return "";
    return value.startsWith("/69f09728a40fc8fdf7c08a6c-preview") ? value : (value.startsWith("/") ? "/69f09728a40fc8fdf7c08a6c-preview" + value : "/69f09728a40fc8fdf7c08a6c-preview/" + value);
  };
    ...
    const response = await fetch(
      `https://app-cms-api-proxy-staging-001.azurewebsites.net/vrio/order_offers`,
      {
        ...
        body: JSON.stringify({
          ...
          order_id: lastOrderId,
          pageId: "wh-TfpAktK8jj4c81TV0l89ZEj4dlnRKh1IHSaioUXYkVQ5UT5MhIo1ALM-YurgF"
        })
      }
    );
...
const processUpsell = async () => {
  ...
  try {
    const orderData = JSON.parse(sessionStorage.getItem("orderData"));
    orderData.pageId = "wh-TfpAktK8jj4c81TV0l89ZEj4dlnRKh1IHSaioUXYkVQ5UT5MhIo1ALM-YurgF";
    const lastOrderId = sessionStorage.getItem("cms_oid");
...
if (typeof validateAndSendToKlaviyo === "function") {
  try {
    ...
    var pageReadyPayload = {
      ...
      data: { pageName: "Upsell 1 Test", pageType: "Upsell" },
    };
